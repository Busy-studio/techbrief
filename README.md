...
사이드바에서 아래 설정을 사용자가 직접 바꿀 수 있습니다.
...
- 기본 direct reference는 해당 레퍼런스 중 실사 기술 몽타주와 부산대학교 소형 라벨 패턴을 대표하는 이미지 2장을 앱에 내장했습니다.
...
기존 Tech Brief와의 유사도를 높이기 위해 기본 스타일에서는 다음을 강하게 제한합니다.

- 실사 사진 2~3장의 넓은 면적을 이용한 자연스러운 블렌딩 우선
- 부산대학교 + 학과/교수명 외 대형 키워드/제목 텍스트 생성 금지
- 부품 콜아웃, 화살표, 리더 라인, 코어층/클래딩층 같은 구조 라벨 금지
- 중앙의 투명 3D 모형, 폭발도, 단면도, 과도한 CGI 제품 렌더 지양
- 강한 네온, 레이저, 사이버펑크, 게임 키아트식 연출 제한
- 실제 피사체 색과 물성을 우선하고 필요한 범위에서만 색조를 통일
- 학교 라벨은 상단 좌/우 중 피사체를 덜 가리는 여백에 작게 배치

새 사용자 스타일 ZIP을 업로드한 경우에는 해당 ZIP 분석 결과가 우선되므로 위 기본 스타일 고정 규칙을 그대로 강제하지 않습니다.
